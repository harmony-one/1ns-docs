...
It leverages IPFS for storage and can be implemented on any EVM compatabile blockchain.

It is targeted to be launched on Harmony which is a prime candidate due to it's fast settlement time (less than 2 seconds) and cheap gas fees. Also there has been significant development of web3 identity and domain management as well as wallet functionality as detailed below.

**Pre-existing work and integration with Harmony Protocol**

Harmony launched the RADICAL initiative[^1] which included many foundational layers for web3 internet domains. These include

1. The ability to manage `.1.country` subdomain[^2]
2. A browser extension for web3 domain redirects[^3]
3. A Web3 backend for managing `.country` domains [^4] [^5] [^9] [^10] [^11]
4. An email alias service [see Appendix D - Email Alias Service] [^6]

Also there has been development of wallet functionality by [modulo](https://modulo.so) to provide secure, frictionless, programmable non-custodial wallet infrastructure, this includes

1. OTP Wallet[^7]
2. SMS Wallet[^8]

This foundational work can leverage and integrate with DEES to offer a complete Digitial Identity Experience as well as the ability to easily manage and transfer crypto assets.

## DEES Ecosystem

**DEES Reference architecture**

Following is a reference architecure based on the components developed as part of the integration with the Harmony Protocol. See [Appendix B - Implementation Strategy](#appendix-b---implementation-strategy) for a detailed component breakdown and development tasks.

![DEES Reference Architecture](/assets/dees-architecture.jpeg "DEES Reference Architecture")

## Self-hosted Mail Server

The self hosted mail server is the foundational component for DEED. It's role is to support SMTP administration and handle the forwarding and managment of emails.
...
![Modulo Components](/assets/modulo-components.jpeg "Modulo Components")
...
![Dmail Architecture](/assets/dmail-architecture.png "Dmail Architecture")
